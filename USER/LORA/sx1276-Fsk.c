...
tFskSettings FskSettings = 
{
    ...
    true,           // CrcOn
    true,           // AfcOn    
    255             // PayloadLength (set payload size to the maximum for variable mode, else set the exact payload length) /*  有explict 模式下,包长度可变   */			
};
...
void SX1276FskInit( void )
{
	...
	SX1276Write( REG_OCP, 0x3f );	/* 默认参数PA最在电流为100mA,当输出20dBm时需要120mA,所以必须配置0x0b存器*/
...
//        PacketTimeout = PacketTimeout + ( PacketTimeout >> 1 ); // Set the Packet timeout as 1.5 times the full payload transmission time
 ...
 //       Preamble2SyncTimer = RxTimeoutTimer = GET_TICK_COUNT( );
        ...
        RxBytesRead = 0;
        RxPacketSize = 0;
        RFState = RF_STATE_RX_RUNNING;//RF_STATE_RX_SYNC;
        break;
...
/*********************************************************************************************************
** Function name:       FSKRxStateEnter
** Descriptions:        进入接收状态
** input parameters:    无
** output parameters:   无
** Returned value:      无
*********************************************************************************************************/
void FSKRxStateEnter (void)
{
//    case RF_STATE_RX_INIT:
        // DIO mapping setup
        if( ( SX1276->RegPacketConfig1 & RF_PACKETCONFIG1_CRC_ON ) == RF_PACKETCONFIG1_CRC_ON ) {
...
//    case RF_STATE_RX_RUNNING:
        if( RxPacketSize > RF_BUFFER_SIZE_MAX ) {
            RFState = RF_STATE_RX_LEN_ERROR;
//            break;
			RxPacketSize = RF_BUFFER_SIZE_MAX;
        }
//#if 1
        if( DIO1 == 1 ) // FifoLevel
        {
...
** Returned value:      无
*********************************************************************************************************/
void FSKRxDataRead (uint8_t *pbuf, uint8_t *size )
{
	if( DIO0 == 1 ) // PayloadReady/CrcOk
	{
		RxTimeoutTimer = GET_TICK_COUNT( );
...
*********************************************************************************************************/
uint8_t FSKTxData (uint8_t *pbuf, uint8_t size )
{
	uint32_t i,j; 
  // Packet DIO mapping setup
        //                           PacketSent,               FifoLevel,              FifoFull,               TxReady
...
//    case RF_STATE_TX_READY_WAIT:
        if( 1 )    // TxReady   DIO3 ==
        {
            ...
            if( ( SX1276->RegPacketConfig1 & RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE ) == RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE )
            {
                SX1276WriteFifo( ( uint8_t* )&TxPacketSize, 1 );     /* 包长度可变,写入包长度   */	
            }
...
//    case RF_STATE_TX_RUNNING:
	...
	i = 3000;
	while ( DIO0 == 0 && 0 != i) { /* 等待发送完成或者发送失败超时,完成后:DIO0为高电平,FSK模式下离开TX状态该中断被清除 */
		i--;	
		for(j = 0; j < 4369; j++);	   /* 约1ms */
	}
	
	SX1276FskSetOpMode( RF_OPMODE_STANDBY );       /* 清发送完成中断 */	
	if (0 == i) {   /* 如果发送失败 */
		return 1;
	} else {        /* 如果发送成功 */	
		return 0;
